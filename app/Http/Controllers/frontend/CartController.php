...
class CartController extends Controller
{
    ...
    public function payment(CheckoutRequest $request){
        ...
        // Cập nhật số lượng đã mua cho từng sản phẩm
        foreach ($this->cart as $product) {
            $dataProduct = ProductModel::find($product['cart_id']);
            $dataProduct->product_purchase += $product['cart_quantity'];
            $dataProduct->save();
        }
        ...
        if($request->order_pay_type == 2){//Xử lý chuyển view sang thanh toán online
            ...
            foreach($this->cart as $val){
             ...
             // Trừ số lượng trong mã giảm giá sau khi thành công thanh toán
            if($this->coupon){
                $coupon = CouponModel::find($this->coupon['coupon_id']);
                $coupon->quantity -= 1;
                $coupon->save();
            }
    ...
    public function paymentCreate(Request $request){
        $vnp_TmnCode = "GG5RYLB2"; //Mã website tại VNPAY 
        $vnp_HashSecret = "ILUMEZQCEJKFNKJOOXTNISOKOKUKOAND"; //Chuỗi bí mật
    ...
    public function paymentReturn(Request $request){
        ...
        if($request->vnp_ResponseCode == "00") {
            ...
            foreach($this->cart as $val){
                ...
                $dataProduct->product_amount = $dataProduct->product_amount - $val['cart_quantity'];
                // Cập nhật số lượng đã mua cho từng sản phẩm
                $dataProduct->product_purchase += $val['cart_quantity'];
