...
class CustomerController extends Controller
{
    ...
    public function customerRegister(Request $request)
    {
        ...
        $request->validate([
            ...
            'user_password' => ['required', 'string', 'min:5', 'max:20', 'regex:/^(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*])[A-Za-z0-9!@#$%^&*]+$/'],
            'user_password_again' => ['required', 'string', 'min:5', 'max:20', 'regex:/^(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*])[A-Za-z0-9!@#$%^&*]+$/'],
            ...
            'user_password.regex' => 'Mật khẩu bắt đầu bằng chữ hoa, bao gồm số và kí tự đặc biệt',
            'user_password_again.same' => 'Mật khẩu xác nhận không khớp',
        ]);

        $data->user_name = $request->user_name;
        $data->user_email = $request->user_email;
        $data->email = $request->user_email; // Thêm dòng này để lưu giá trị email vào cả hai cột
